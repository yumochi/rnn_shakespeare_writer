...
term_fn="Result/"+started_datestring+".txt"
init="PyTorch training run\n\n"+str(started_datestring)+"--------------------\n\n"
with open(term_fn, 'w+') as f:
    f.write(init)
...
parser.add_argument('--batch_size', type=int, default=20, metavar='N',
                    help='batch size')
parser.add_argument('--bptt', type=int, default=38,
...
para = ("\nData folder: "+str(args.data)+"\nModel folder: "+str(args.model_dir)+"\n\nSize of word embeddings: "+str(args.emsize)+"\nNumber of hidden units per layer: "+str(args.nhid)+"\nModel: "+str(args.model)+"\nNumber of layers: "+str(args.nlayers)+"\nBatch size: "+str(args.batch_size)+"\nDropout: "+str(args.dropout)+"\n")
print (para)
print("="*45)
with open(term_fn, 'w+') as f:
    f.write(para)
...
try:
    for epoch in range(1, args.epochs+1):
        ...
        #ADDED
        t1='-' * 89
        t2='\n| end of epoch {:3d} | time: {:5.2f}s | valid loss {:5.4f} | valid ppl {:8.2f}'.format(epoch, (time.time() - epoch_start_time), val_loss, math.exp(val_loss))
        #t3='-' * 89
        ta=t1+t2
        print(ta)
        ...
        if not best_val_loss or val_loss < best_val_loss:
            ...
            # BUT THN when it hits bottom, levitate it.. anneal, cool, heat, cool heat
            if val_loss<0.05:
                lr = args.lr/2
            else:
                lr /= 1.5 #2
